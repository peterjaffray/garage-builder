...
const RoofDesignStep: React.FC<StepComponentProps> = ({ formData, onUpdate, onNext, onBack }) => {
  const standardRoofs = [
    ...
    { value: 'gable2', label: 'Gable Style 2', description: 'Modified gable with different pitch' }
  ];
  ...
  const availableRoofs = formData.atticStorage === 'no' 
    ? [...standardRoofs, ...premiumRoofs]
    : [...standardRoofs, ...premiumRoofs];
